<?php
/**
  * @var \App\View\AppView $this
  */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Child'), ['action' => 'edit', $child->id_child]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Child'), ['action' => 'delete', $child->id_child], ['confirm' => __('Are you sure you want to delete # {0}?', $child->id_child)]) ?> </li>
        <li><?= $this->Html->link(__('List Children'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Child'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Users'), ['controller' => 'Users', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New User'), ['controller' => 'Users', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="children view large-9 medium-8 columns content">
    <h3><?= h($child->name) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('User') ?></th>
            <td><?= $child->has('user') ? $this->Html->link($child->user->name, ['controller' => 'Users', 'action' => 'view', $child->user->id_user]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id Child') ?></th>
            <td><?= $this->Number->format($child->id_child) ?></td>
        </tr>
    </table>
    <div class="row">
        <h4><?= __('Name') ?></h4>
        <?= $this->Text->autoParagraph(h($child->name)); ?>
    </div>
</div>
